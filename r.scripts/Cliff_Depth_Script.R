...
library(dplyr)
############################
setwd("C:/users/aldoa/Desktop/NDS_MEGA/Cliff_September/")
nds.data = read.csv("Cliff_Lake_Sep_5m.csv", header = T)
...
all.data.no3p = nds.data[nds.data$sample == "N+P",]


boxplot((all.data.c$chla), (all.data.p$chla), (all.data.no3$chla), (all.data.no3p$chla), (all.data.nh4$chla), (all.data.nh4p$chla),
...
############################
plotNormalHistogram(nds.data$chla)
plotNormalHistogram(log10(nds.data$chla))
nds.data_tuk = transformTukey(nds.data$chla,plotit=TRUE) #Tukey transformation
plotNormalHistogram(nds.data_tuk)
############################
qqPlot(nds.data$chla)
qqPlot(notransformmodel)
qqPlot(log10(nds.data$chla))
qqPlot(nds.data_tuk)

############################
notransformmodel = lm(nds.data$chla ~ nds.data$sample, 
                      data=nds.data)
residuals.notransform = residuals(notransformmodel)
###
logtransformmodel = lm(log10(nds.data$chla) ~ nds.data$sample,
                       data=nds.data)
...
#############################
shapiro.test(nds.data$chla)
shapiro.test(residuals.notransform)
shapiro.test(log10(nds.data$chla))
...
nds.data$logtransform <- log10(nds.data$chla)

M2<-gls(chla~sample, data=nds.data)
...
DunnettTest(x=nds.data$chla, g=nds.data$sample)
...
M2Log
ggplot(nds.data_bp, aes(x=sample, y=chla)) + 
